...
| **Answer model** | OpenAI or OpenRouter | Writes the grounded, cited answer and original code/simulations. |
...
|------|--------|-------|----------------|
| **Answer model** | mixed / inconsistent | **OpenAI / OpenRouter** (switchable in the UI) | A single provider interface with reliable cloud models; a built-in eval harness scores any model you pick. |
...
| **Answer LLM** | OpenAI or OpenRouter | Other frontier models (Claude, Gemini) | Top-tier answer quality; the model is switchable in the UI and an eval harness compares them. ✅ |
