...
Run from the project root (Oracle DB must be up and papers indexed):
    ...
    python -m backend.evaluation.evaluate_llm --models "openrouter:qwen/qwen3-32b,openrouter:deepseek/deepseek-v4-pro,ollama:llama3.2:3b"

    # Add an LLM-as-judge correctness score (costs extra API calls)
    python -m backend.evaluation.evaluate_llm --judge --judge-model openrouter:deepseek/deepseek-v4-pro
...
def evaluate_model(spec: Optional[str], questions: List[Dict[str, Any]],
    ...
    if not provider.is_available:
        print("  [skipped] provider not available (missing key, or Ollama not running).")
